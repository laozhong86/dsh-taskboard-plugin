...
async function execute(parsed, overrides) {
  ...
  if (!allowedOptions) {
    throw usageError(
      "Expected one of: project list/create/map, issue list/get/create/update/move/archive/restore/relation, comment list/add/update/delete, attachment download/upload, context current",
    );
  }
  ...
  const api = createApiClient(overrides, {
    baseUrl: await resolveTaskboardBaseUrl(env, overrides),
  });
  switch (command) {
    case "project list":
...
function resolveThreadId(options, overrides) {
  ...
  if (typeof value !== "string" || value.trim().length === 0) {
    throw usageError("Session attribution requires --thread-id or TASKBOARD_THREAD_ID");
  }
