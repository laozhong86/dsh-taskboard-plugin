...
import { spawnCodexTurn } from "./agent-turn-process.mjs";
...
function buildPrompt(project, tasks) {
  ...
  return [
    "你是项目总结助手。请根据下面的任务面板快照，为项目负责人写一段项目总结。",
  ...
  async #generate(projectId, active) {
    try {
      ...
      const { child, completion } = spawnCodexTurn({
        ...
        onRawEvent: (event) => {
          ...
          if (event.type === "turn.failed" || event.type === "error") {
            terminalError = String(event.error?.message ?? event.message ?? "项目总结生成失败");
          }
        },
      });
      active.child = child;
      ...
      if (result.exitCode !== 0 || terminalError) {
        throw new Error(terminalError || `Agent 退出码 ${result.exitCode}`);
      }
      if (!generatedSummary) throw new Error("Agent 没有返回项目总结");
