...
export function buildTaskboardAutomationPrompt(request) {
  ...
  const executionInstructions = remoteProject
    ? [
        `本自动化仅在本机作为任务面板控制器运行；实际开发必须派发到 SSH 远程项目。导入项目的基础 identity 是 projectId=${JSON.stringify(request.codexProjectId)}、hostId=${JSON.stringify(request.codexHostId)}、workspacePath=${JSON.stringify(request.workspacePath)}；同一保存主机当前可用的精确远程项目映射是 ${JSON.stringify(remoteProjects)}。不要在当前本地自动化会话修改项目文件。`,
        ...
        "认领成功后，已有完整 threadBinding 时，只能使用其保存的 threadId 和 codexHostId 调用 send_message_to_thread；若保存的主机或会话不可用，使用 comment add 记录错误，再用 ownedVersion、完整保存 binding 和显式 --if-version 将议题移动到 blocked；409 冲突时立即停止，不得在当前自动化目标创建替代会话。只有未绑定议题才使用 create_thread 创建远程任务，target 必须是 {type:\"project\",projectId:actualTarget.codexProjectId,environment:{type:\"local\"}}，首次 identity 必须使用 actualTarget 的 projectId、kind=\"remote\"、hostId 和 workspacePath。发送给远程会话的指令必须包含议题编号、标题、完整描述、全部评论和开发上下文，并说明远程会话不运行 taskctl，只需完成实现、验证并返回改动、结果和剩余风险。",
    ...
    : [
        ...
        "若首次 issue get 返回 threadId，议题已绑定原会话：不要在当前自动化会话认领；使用 send_message_to_thread 向原会话发送继续处理指令，由原会话按上述协议判断和认领，然后结束当前自动化会话。若没有 threadId，则在当前自动化会话处理。",
  ...
  return [
    `[$taskboard](${request.skillPath}) Taskboard 每 ${request.intervalMinutes} 分钟检查任务面板中的「${request.projectName}」项目（项目 ID：${request.taskboardProjectId}，项目目录：${request.workspacePath}）。`,
    `本轮所有 taskctl 操作都使用完整命令前缀 ${taskctlCommand}，不要使用 PATH 中的 taskctl。`,
    `开始时先运行 ${taskctlCommand} issue list --project ${request.taskboardProjectId} --status todo --json。若没有 todo，使用 automation_update 将名为「${automationName}」的当前自动化设为 PAUSED，保留其他字段，然后结束；不要创建或打开新的任务会话。`,
    ...executionInstructions,
    `本次处理或交接后，再次运行 ${taskctlCommand} issue list --project ${request.taskboardProjectId} --status todo --json。若没有 todo，使用 automation_update 将名为「${automationName}」的当前自动化设为 PAUSED，保留其他字段，避免后续创建空会话。`,
  ].join("\n");
}
...
function buildTaskctlCommand(request) {
  ...
  const runtimeFilePath = process.env.TASKBOARD_RUNTIME_FILE;
